...
func pushSMS(c *gin.Context) {
	...
	if len(req.Text) > 800 {
		...
		smsCount := int(math.Ceil(float64(len(req.Text)) / 800))
		for len(req.Text[pivot:]) > 800 {
			udh = fmt.Sprintf("050003%v%02d%02d", refCode, smsCount, i)
			log.Println("Multiple SMS UDH : ", udh)
			response, err = sendReq("demoindo1", "http9876", req.To, udh, req.From, req.Text[pivot:pivot+800], dlrURL)
			if err != nil {
				log.Fatalf("Error on sending SMS %d: %v", i, err)
			}
			pivot += 800
			i++
		}
		udh = fmt.Sprintf("050003%v%02d%02d", refCode, smsCount, i)
		log.Println("Multiple SMS UDH : ", udh)
		response, err = sendReq("demoindo1", "http9876", req.To, udh, req.From, req.Text[pivot:len(req.Text)], dlrURL)
		if err != nil {
			log.Fatalf("Error on sending last SMS: %v", err)
		}
	} else {
		log.Println("Single SMS UDH : ", udh)
		response, err = sendReq("demoindo1", "http9876", req.To, udh, req.From, req.Text, dlrURL)
		if err != nil {
			log.Fatalf("Error on sending request to VFirst %v", err)
		}
...
func sendReq(uname, pass, to, udh, from, text, dlrURL string) (string, error) {
	url := fmt.Sprintf("http://www.myvaluefirst.com/smpp/sendsms?username=%s&password=%s&to=%s&udh=%s&from=%s&text=%s&dlr-url=%s",
		"demoindo1", "http9876", to, url.PathEscape(udh), from, url.PathEscape(text), url.QueryEscape(dlrURL))
